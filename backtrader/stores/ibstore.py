...
class IBApi(EWrapper, EClient):
    ...
    @logibmsg
    def updatePortfolio(
        self, contract, position, marketPrice, marketValue, averageCost, unrealizedPNL, realizedPNL, accountName
    ):
        """This function is called only when reqAccountUpdates on
        EEClientSocket object has been called."""
...
class IBStore(with_metaclass(MetaSingleton, object)):
    ...
    def reqHistoricalTicksEx(
        self, contract, enddate=None, begindate=None, what=None, useRTH=False, tz="", tickerId=None
    ):
        """
        Extension of the raw reqHistoricalData proxy, which takes two dates
        rather than a duration, barsize and date
    ...
    def reqHistoricalTicks(self, contract, enddate, begindate, what=None, useRTH=False, tz=""):
    ...
    def makecontract(
        self, symbol, sectype, exch, curr, expiry="", strike=0.0, right="", mult=1, primaryExch=None, localSymbol=None
    ):
    ...
    def updatePortfolio(
        self, contract, pos, marketPrice, marketValue, averageCost, unrealizedPNL, realizedPNL, accountName
    ):
        # Lock access to the position dicts. This is called in sub-thread and
        # can kick in at any time
        with self._lock_pos:
